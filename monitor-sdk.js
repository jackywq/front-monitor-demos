...
(function (global, factory) {
    ...
    const utils = {
        ...
        // 生成唯一ID
        generateId: () => {
            return 'monitor_' + Math.random().toString(36).substr(2, 9) + '_' + utils.now();
        },
    ...
    class DataQueue {
        ...
        report(data) {
            ...
            if (navigator.sendBeacon) {
                ...
                // 降级方案：使用Image对象
                const img = new Image();
                img.src = this.config.reportUrl + '?data=' + encodeURIComponent(utils.safeStringify(reportData));
            }
        }
        ...
        shouldIgnoreError(error, event) {
            ...
            return this.config.ignoreErrors.some(ignorePattern => {
                if (typeof ignorePattern === 'string') {
                    return errorMessage.includes(ignorePattern) || errorUrl.includes(ignorePattern);
                } else if (ignorePattern instanceof RegExp) {
                    return ignorePattern.test(errorMessage) || ignorePattern.test(errorUrl);
                }
                return false;
            });
        }
    ...
    class PerformanceMonitor {
        ...
        collectPerformance() {
            ...
            const performanceData = {
                type: 'performance',
                navigationType: navigation ? this.getNavigationType(navigation.type) : 'unknown',
